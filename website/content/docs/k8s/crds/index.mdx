...
name of the resource in Kubernetes is ignored when synced to Consul.
For other resources, the name of the resource
...
into depends on the configuration being used. In general, you should create your
...
1. **Mirroring** - The Kubernetes namespace will be "mirrored" into Consul, i.e.
   ...
   in the Consul namespace `web-ns`. Similarly, a `ServiceDefaults` custom resource with
   the name `web` in Kubernetes namespace `web-ns` will configure that same service in Consul.
...
1. **Mirroring with prefix** - The Kubernetes namespace will be "mirrored" into Consul
   with a prefix added to the name of the namespace in Consul. For example,
   if the prefix is `k8s-` then service `web` in Kubernetes namespace `web-ns` will be registered as service `web`
   in the Consul namespace `k8s-web-ns`. Similarly, a `ServiceDefaults` custom resource with
   name `web` in Kubernetes namespace `web-ns` will configure that same service in Consul.
...
1. **Single destination namespace** - The Kubernetes namespace is ignored and all services
   will be registered into a single Consul namespace. For example, if the destination Consul
   ...
   service is running in Kubernetes namespace `web-ns`. This is because the `ServiceDefaults`
   resource ends up being registered into the Consul namespace `my-ns`.
   ...
   be created in two Kubernetes namespaces, the last resource created will not be synced.

#### ServiceIntentions Special Case <EnterpriseAlert inline />
