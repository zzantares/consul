...
- `global` ((#v-global)) - Holds values that affect multiple components of the chart.
  ...
  - `secretsBackend` ((#v-global-secretsbackend)) - secretsBackend is used to configure Vault as the secrets backend for the Consul on Kubernetes installation.
    ...
    - `vault` ((#v-global-secretsbackend-vault))
      ...
      - `consulSnapshotAgentRole` ((#v-global-secretsbackend-vault-consulsnapshotagentrole)) (`string: ""`) - <EnterpriseAlert inline /> The Vault role for the Consul client snapshot agent.
        ...
        have a policy with read capabilities for the following secrets:
        -  snapshot agent config defined by `client.snapshotAgent.configSecret.secretName`.
        To discover the service account name of the Consul client, run
            ```shell-session
      ...
      - `adminPartitionsRole` ((#v-global-secretsbackend-vault-adminpartitionsrole)) (`string: ""`) - <EnterpriseAlert inline /> A Vault role to allow Kubernetes job that creates a Consul partition for this Helm chart (`partition-init`) to read Vault secret for the partition ACL token.
  ...
  - `acls` ((#v-global-acls)) - Configure ACLs.
    ...
    - `partitionToken` ((#v-global-acls-partitiontoken)) - artitionToken references a Vault secret containing the ACL token to be used in non-default partitions.
      ...
      We will use the value of the secret stored in Vault to create an ACL token in Consul with the value of the secret as the secretID for the token.
  ...
  - `federation` ((#v-global-federation)) - Configure federation.
    ...
    - `k8sAuthMethodHost` ((#v-global-federation-k8sauthmethodhost)) (`string: ""`) - If you are setting `global.federation.enabled` to true and are in a secondary datacenter,
...
- `client` ((#v-client)) - Values that configure running a Consul client on Kubernetes nodes.
  ...
  - `snapshotAgent` ((#v-client-snapshotagent)) - <EnterpriseAlert inline /> Values for setting up and running snapshot agents
    ...
    - `configSecret` ((#v-client-snapshotagent-configsecret)) - A Kubernetes or Vault secret that should be manually created to contain the entire
      ...
      - `secretName` ((#v-client-snapshotagent-configsecret-secretname)) (`string: null`) - The name of the Kubernetes secret or Vault secret path that holds the snapshot agentconfig.

      - `secretKey` ((#v-client-snapshotagent-configsecret-secretkey)) (`string: null`) - The key within the Kubernetes secret or Vault secret key that holds the snapshot agentconfig.
