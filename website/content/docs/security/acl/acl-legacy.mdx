...
1. The datacenter listing operation of the
...
1. Performing [anti-entropy](/docs/internals/anti-entropy) syncing, in particular reading the node metadata and services registered with the catalog
1. Reading and writing the special `_rexec` section of the KV store when executing [`consul exec`](/commands/exec) commands
...
1. An ACL master token of "b1gs33cr3t"; see below for an alternative using the [/v1/acl/bootstrap API](/api/acl/acl#bootstrap-acls)
1. A default policy of "deny" which means we are in allowlist mode
1. A down policy of "extend-cache" which means that we will ignore token TTLs
   during an outage
...
1. Providing an ACL token with service and check definitions at
...
1. Enable ACL replication in all datacenters to allow continuation of service
   ...
   endpoint.
1. Turn down the old authoritative datacenter servers.
1. Rolling restart the agents in the target datacenter and change the
...
1. Rolling restart the agents in other datacenters and change their `acl_datacenter`
