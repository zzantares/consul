...
Reading a `service-resolver` config entry requires `service:read` on the service with the same name as the config entry.

Creating, updating, or deleting a `service-resolver` config entry requires
`service:write` on the service with the same name as the config entry and `service:read` on any other service referenced by
