...
func processTestFile(pass *analysis.Pass, f *ast.File, allowedPackages []string) {
	...
	// insert validation for test structure

	if false {
		pass.Reportf(f.Name.Pos(), "package %s does not contain a valid loop structure (either over a slice or a map) for its test cases`", packageName)
	}
}
...
func NewAnalyzer() *analysis.Analyzer {
	...
	return &analysis.Analyzer{
		Name:  "teststructure",
		Doc:   "linter that ensures your tests follow a common pattern",
		Flags: fs,
