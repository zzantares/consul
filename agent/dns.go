...
func (d *DNSServer) nameservers(cfg *dnsConfig, edns bool, maxRecursionLevel int, req *dns.Msg) (ns []dns.RR, extra []dns.RR) {
	out, err := d.lookupServiceNodes(cfg, d.agent.config.Datacenter, structs.ConsulServiceName, "", structs.DefaultEnterpriseMeta(), false, false, maxRecursionLevel)
	if err != nil {
...
func (d *DNSServer) lookupServiceNodes(cfg *dnsConfig, datacenter, service, tag string, entMeta *structs.EnterpriseMeta, connect, ingress bool, maxRecursionLevel int) (structs.IndexedCheckServiceNodes, error) {
...
func (d *DNSServer) serviceLookup(cfg *dnsConfig, network, datacenter, service, tag string, entMeta *structs.EnterpriseMeta, connect, ingress bool, req, resp *dns.Msg, maxRecursionLevel int) {
	out, err := d.lookupServiceNodes(cfg, datacenter, service, tag, entMeta, connect, ingress, maxRecursionLevel)
	if err != nil {
		d.logger.Error("rpc error", "error", err)
		resp.SetRcode(req, dns.RcodeServerFailure)
		return
