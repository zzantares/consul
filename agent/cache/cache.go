...
func (c *Cache) Get(t string, r Request) (interface{}, ResultMeta, error) {
	...
	return c.getWithIndex(newRequest(tEntry, r))
}

// request contains the arguments for a Get request. It is used in place of
...
func newRequest(tEntry typeEntry, r Request) request {
	return request{
		Fetch: func(opts FetchOptions) (FetchResult, error) {
...
func (c *Cache) getWithIndex(r request) (interface{}, ResultMeta, error) {
	...
	if info.Key == "" {
		...
		result, err := r.Fetch(FetchOptions{MinIndex: info.MinIndex})
