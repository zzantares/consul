...
func (c *Cache) Notify(
	...
	if tEntry.Type.SupportsBlocking() {
		go c.notifyBlockingQuery(ctx, newRequest(tEntry, r), correlationID, ch)
		return nil
	}
	...
	go c.notifyPollingQuery(ctx, newRequest(tEntry, r), correlationID, ch)
	return nil
}

func (c *Cache) notifyBlockingQuery(ctx context.Context, r request, correlationID string, ch chan<- UpdateEvent) {
...
func (c *Cache) notifyPollingQuery(ctx context.Context, r request, correlationID string, ch chan<- UpdateEvent) {
	index := uint64(0)
	failures := uint(0)
