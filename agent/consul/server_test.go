...
func newServer(t *testing.T, c *Config) (*Server, error) {
	...
	rpcRouter := router.NewRouter(logger, c.Datacenter, fmt.Sprintf("%s.%s", c.NodeName, c.Datacenter), nil)
	srv, err := NewServer(c,
		WithLogger(logger),
...
func TestServer_CALogging(t *testing.T) {
	...
	rpcRouter := router.NewRouter(logger, "dc1", fmt.Sprintf("%s.%s", "nodename", "dc1"), nil)
