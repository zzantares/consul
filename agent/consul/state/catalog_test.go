...
func TestStateStore_IngressGatewaysForService_Prefix(t *testing.T) {
	s := testStateStore(t)
	...
	testRegisterService(t, s, 5, "node2", "ingress2")
	testRegisterService(t, s, 6, "node1", "ingress3")
	testRegisterService(t, s, 7, "node1", "fooaaa")
	testRegisterService(t, s, 8, "node2", "foobbb")
	testRegisterService(t, s, 9, "node2", "bar")
		...
		Listeners: []structs.IngressListener{
			{
				Port:     1111,
				Protocol: "tcp",
				ServicePrefixes: []structs.IngressService{
					{
						Prefix: "foo",
					},
				},
	...
	// Lookup for service1 first.
	{
		idx, results, err := s.IngressGatewaysForService(ws, "fooaaa", nil)
		require.NoError(err)
		require.Equal(uint64(5), idx)
		require.Len(results, 2)
		require.Equal("ingress1", results[0].Service.ID)
	...
	// Lookup for service2, should only get one ingress gateway.
	{
		idx, results, err := s.IngressGatewaysForService(ws, "foobbb", nil)
		require.NoError(err)
		require.Equal(uint64(4), idx)
		require.Len(results, 1)
		require.Equal("ingress1", results[0].Service.ID)
	...
	// Lookup for service3, should only get one ingress gateway.
	{
		idx, results, err := s.IngressGatewaysForService(ws, "bar", nil)
		require.NoError(err)
		require.Equal(uint64(0), idx)
		require.Len(results, 0)
	}
