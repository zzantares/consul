...
func (c *Catalog) ListServices(args *structs.DCSpecificRequest, reply *structs.IndexedServices) error {
	...
	return c.srv.blockingQuery(
		...
		func(ws memdb.WatchSet, state *state.Store) error {
			...
			if len(args.NodeMetaFilters) > 0 {
				reply.Index, serviceNodes, err = state.ServicesByNodeMeta(ws, args.NodeMetaFilters, &args.EnterpriseMeta)
			} else {
				reply.Index, serviceNodes, err = state.Services(ws, &args.EnterpriseMeta)
			}
			if err != nil {
				return err
			}
