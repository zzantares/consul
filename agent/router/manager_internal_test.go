...
func testManager() (m *Manager) {
	...
	m = New(logger, shutdownCh, &fauxSerf{numNodes: 16384}, &fauxConnPool{}, "", NoOpServerTracker{})
	return m
}
...
func testManagerFailProb(failPct float64) (m *Manager) {
	logger := GetBufferedLogger()
	shutdownCh := make(chan struct{})
	m = New(logger, shutdownCh, &fauxSerf{}, &fauxConnPool{failPct: failPct}, "", NoOpServerTracker{})
	return m
}
...
func TestManagerInternal_refreshServerRebalanceTimer(t *testing.T) {
	...
	for _, s := range clusters {
		m := New(logger, shutdownCh, &fauxSerf{numNodes: s.numNodes}, &fauxConnPool{}, "", NoOpServerTracker{})
