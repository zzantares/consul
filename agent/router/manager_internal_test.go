...
func TestManager_refreshServerRebalanceTimer(t *testing.T) {
	...
	testCases := []testCase{
		{clients: 0, servers: 3},
		{clients: 1, servers: 0}, // partitioned cluster
		{clients: 1, servers: 3},
		{clients: 2, servers: 3},
		{clients: 100, servers: 0}, // partitioned
		{clients: 100, servers: 1}, // partitioned
		{clients: 100, servers: 3},
		{clients: 1024, servers: 1}, // partitioned
		{clients: 1024, servers: 3}, // partitioned
		{clients: 1024, servers: 5},
		{clients: 16384, servers: 1, expected: 4*time.Minute + 16*time.Second}, // partitioned
		{clients: 16384, servers: 2}, // partitioned
		{clients: 16384, servers: 3}, // partitioned
		{clients: 16384, servers: 5},
		{clients: 65535, servers: 0}, // partitioned
		{clients: 65535, servers: 1, expected: 17*time.Minute + 3984375000},
		{clients: 65535, servers: 2, expected: 8*time.Minute + 31992187500}, // partitioned
		{clients: 65535, servers: 3, expected: 5*time.Minute + 41328125000}, // partitioned

		{clients: 65535, servers: 5, expected: 3*time.Minute + 24796875000}, // partitioned
		{clients: 65535, servers: 7},
		{clients: 1000000, servers: 1, expected: 4*time.Hour + 20*time.Minute + 25*time.Second},         // partitioned
		{clients: 1000000, servers: 2, expected: 2*time.Hour + 10*time.Minute + 12500*time.Millisecond}, // partitioned
		{clients: 1000000, servers: 3, expected: 86*time.Minute + 48333333333},                          // partitioned
		{clients: 1000000, servers: 5, expected: 52*time.Minute + 5*time.Second},                        // partitioned
		{clients: 1000000, servers: 11, expected: 23*time.Minute + 40454545454},                         // partitioned
		{clients: 1000000, servers: 19, expected: 13*time.Minute + 42368421052},
	}
	...
	for _, tc := range testCases {
		...
		msg := fmt.Sprintf("nodes=%d, servers=%d", tc.clients, tc.servers)
		if tc.expected != 0 {
			assert.Equal(t, tc.expected, delay, msg)
			continue
		}
