...
func testManager(t testing.TB) (m *router.Manager) {
	...
	m = router.New(logger, shutdownCh, &fauxSerf{}, &fauxConnPool{}, "", router.NoOpServerTracker{})
	return m
}
...
func testManagerFailProb(t testing.TB, failPct float64) (m *router.Manager) {
	logger := testutil.Logger(t)
	shutdownCh := make(chan struct{})
	m = router.New(logger, shutdownCh, &fauxSerf{}, &fauxConnPool{failPct: failPct}, "", router.NoOpServerTracker{})
	return m
}
...
func testManagerFailAddr(t testing.TB, failAddr net.Addr) (m *router.Manager) {
	logger := testutil.Logger(t)
	shutdownCh := make(chan struct{})
	m = router.New(logger, shutdownCh, &fauxSerf{}, &fauxConnPool{failAddr: failAddr}, "", router.NoOpServerTracker{})
	return m
}
